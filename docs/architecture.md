...
`cmd/butter-daemon` 是 daemon client 进程，主动连接服务端 gRPC `DaemonConnectorService.Connect`，注册自身能力，接收任务并通过本地 executor 执行。coding agent 类工具优先通过通用 ACP executor 接入，例如 `opencode acp`；shell executor 仍作为独立能力保留。
...
配置中 `RemoteAgent.protocol = REMOTE_AGENT_PROTOCOL_DAEMON` 时，`internal/agent` 会创建 `daemon.Bridge`，并按 `daemon_capability` 从 `daemon.Registry` 查找在线连接。Bridge 把 ADK invocation 转成 `DaemonTask`，等待 daemon 回传 terminal update 后生成 ADK final event。daemon 内部可把该 capability 绑定到 ACP executor（如 `opencode acp`）或 shell executor。
