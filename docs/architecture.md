...
`AuthService` / `WorkspaceService` / `DashboardService` / `DaemonService` 不要求 `X-Workspace-ID`；`SessionService` 的 session CRUD 按 `app_name` + `user_id` + `session_id` 查询，调用 `ReplySession` 时应带 `X-Workspace-ID`，让 runner 在正确 workspace 解析 agent。
...
- ADK memories。
- 配置仓库：`config_agents` / `config_mcpservers` / `config_remoteagents` / `config_channels` / `config_modelproviders` / `config_notifygroups`，`_id` 为 `"{workspace_id}:{name}"` 复合键，并对 `(workspace_id, name)` 建索引。
