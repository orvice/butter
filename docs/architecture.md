...
所有配置实体（`Agent` / `AgentChannel` / `MCPServer` / `RemoteAgent` / `ModelProvider` / `NotifyGroup` / `CronJob` / `APIToken`）、内容实体（`AgentFileSpace` / `AgentFile` / `ForumThread` / `ForumPost`）以及运行时记录（`Invocation` / `CronExecution`）都归属于一个 workspace。`Workspace` 自身和 `WorkspaceMember` 由 `WorkspaceService` 管理，持久化为 MongoDB 的 `workspaces` 和 `workspace_members` 集合（`(workspace_id, user_id)` 唯一索引）。
...
Persistence
├── MongoDB: session, memory, config, cron, invocations, api_tokens, workspaces, workspace_members, agent files, forum
...
Daemon gRPC `Connect` 不接受 root token 或普通 API token，只接受 `DaemonService.CreateDaemonCredential` 为已有 `DaemonConfig` 签发的 daemon credential。
...
Daemon 是 workspace 配置资源：先通过 `DaemonService.CreateDaemonConfig` 写入 `config_daemons`，再通过 `CreateDaemonCredential` 签发 `API_TOKEN_KIND_DAEMON` + `daemon:connect` token。`GRPCHandler` 在第一条 register 消息后校验 token，token 的 workspace 与 daemon_id 为 authoritative 值；daemon 自报 workspace 不一致会被拒绝，自报 capabilities 会被过滤到 `DaemonConfig.allowed_capabilities`。

`daemon.Registry` 以 `workspace_id -> daemon_id -> connection` 管理连接。配置中 `RemoteAgent.protocol = REMOTE_AGENT_PROTOCOL_DAEMON` 时，`internal/agent` 会创建带 workspace 的 `daemon.Bridge`，并按 `(workspace_id, daemon_capability)` 从 registry 查找在线连接。Bridge 把 ADK invocation 转成带 `workspace_id` 的 `DaemonTask`，等待 daemon 回传 terminal update 后生成 ADK final event。daemon 内部可把该 capability 绑定到 ACP executor（如 `opencode acp`）或 shell executor。
...
- `api_tokens`：DB-stored API tokens（带 `workspace_id` + `secret_hash` + `prefix` + `kind` + `scopes` + optional `expires_at` / `daemon_id` + `last_used_at` + `revoked`）。
