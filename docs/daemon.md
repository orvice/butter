...
Butter daemon 是一个反向连接的远程执行面。`cmd/butter` 负责启动服务端的 daemon
ConnectRPC/h2c 入口，`cmd/butter-daemon` 主动连接该入口、注册 runtime，然后等待服务端下发
daemon-backed RemoteAgent 的任务。

当前实现的核心约束：

- 服务端 daemon 入口默认监听 `:9090`，可通过 `grpc_port` 配置。
...
`--url` 可以传裸 `host:port`，也可以传 `http://...` 或 `https://...`。当前 daemon client
会把裸 `host:port` 视为 cleartext h2c 地址。
...
```bash
export BUTTER_DAEMON_URL=localhost:9090
...
如果 Butter server 和 daemon 在同一个 Docker network 中，`BUTTER_DAEMON_URL` 应写成服务名：
