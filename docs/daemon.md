...
`--url` 传 ConnectRPC base URL，例如本地 cleartext h2c 的 `http://localhost:8081/api`，
或生产环境中支持 HTTP/2 的 `https://butter.example.com/api`。daemon client
会自动追加 `/agents.v1.DaemonConnectorService/Connect`，不要把方法路径手动拼进去。
没有 scheme 的值会被视为 cleartext HTTP 地址，需要入口支持 h2c。

如果前面有反代，daemon connector 的上游应指向 `:8081` h2c listener，或由反代直接提供
HTTP/2 到应用侧；这个长连接路径必须关闭 buffering 并拉长超时：
...
```bash
export BUTTER_DAEMON_URL=http://localhost:8081/api
