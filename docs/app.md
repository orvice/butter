...
- Proto TS 绑定通过 `buf.build/bufbuild/es`（`include_imports: true`）输出到 `front/src/gen/`，service 定义和 message 类型都包含在内（连 protoc-gen-connect-es 都不需要 — connect-es v2 直接消费 `GenService`）。每个 service 一个 `front/src/api/*.ts`，用 `makeClient(XxxService)` 拿到类型化 client；共享 `transport.ts` 注入 `Authorization` / `X-Workspace-ID` header 并处理 401 跳登录。手写 `front/src/types/api.ts` 仍保留 snake_case 形状作为消费层 boundary，避免改动散布到全部 page 组件 — server JSON wire format 通过 `connectx.HandlerOptions()` 强制 `UseProtoNames=true` 输出 snake_case，和原 Twirp 兼容。
