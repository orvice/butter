...
- **反向连接架构**：daemon client（`cmd/butter-daemon --url <butter-base-url>/api --token <runtime-token>`）主动连接服务端 `DaemonConnectorService.Connect`，用 workspace-scoped runtime token 鉴权，注册 `DaemonInfo` 与支持的 `acp_runtimes`。
...
- **统一入口**：daemon worker 连接与 dashboard API 共用主 HTTP 服务端口，路径为 `/api/agents.v1.DaemonConnectorService/Connect`。
