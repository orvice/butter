...
- **Workspace 实体**：所有 Agent / Channel / MCP Server / Remote Agent / Model Provider / Notify Group / Agent File / Forum / Cron Job / API Token / Invocation / Cron Execution 必须归属一个 workspace。`WorkspaceService` 暴露 workspace 与成员（`owner` / `admin` / `member`）的 CRUD。
...
- **Daemon 自带 workspace**：daemon 是配置仓库中的 `DaemonConfig` 资源，daemon credential 绑定 workspace + daemon_id；同名 capability 可在不同 workspace 中复用。
...
- **Daemon 协议**：`REMOTE_AGENT_PROTOCOL_DAEMON`，在当前 workspace 内通过 `daemon_capability` 查找在线 daemon 连接，由 `daemon.Bridge` 把 ADK invocation 转成带 `workspace_id` 的 `DaemonTask`，等 daemon 回传 terminal update 后生成 ADK final event。
- **断连/取消**：连接断开时活跃任务会收到失败更新；context 取消时下发 `CancelTask`。

## 5. Daemon 执行面

- **反向连接架构**：daemon client（`cmd/butter-daemon`）主动连接服务端 gRPC `DaemonConnectorService.Connect`，用 workspace-scoped daemon credential 鉴权，注册 `DaemonInfo` 与能力。
- **DB 配置优先**：daemon 必须先通过 `DaemonService.CreateDaemonConfig` 写入 workspace 配置，再通过 `CreateDaemonCredential` 生成 worker credential。服务端用 credential 决定 authoritative workspace + daemon_id，并把自报 capabilities 过滤到 `allowed_capabilities`。
- **任务下发与执行**：服务端通过 `daemon.GRPCHandler` / `Registry` / `Connection` / `Bridge` 把任务下发到 daemon，daemon 通过本地 ACP/shell executor 执行后回传 `DaemonTaskUpdate`。opencode 等 coding agent 通过 `opencode acp` 这类 ACP stdio 入口接入。
...
- `DaemonService`：`ListDaemonConfigs` / `GetDaemonConfig` / `CreateDaemonConfig` / `UpdateDaemonConfig` / `DeleteDaemonConfig` / `CreateDaemonCredential` / `ListDaemons` / `GetDaemon` / `ListDaemonTasks`（含 step+progress+elapsed）/ `CancelDaemonTask` / `GetBridgeDiagnostics`（CPU / 内存 / 延迟样本）。
...
- **MongoDB**（默认数据库 `butter`，可通过 `mongo_db` 配置）：
  ...
  - `api_tokens`：DB-stored API tokens（哈希 + prefix + workspace_id + kind + scopes + optional expires_at / daemon_id）
...
- **DB-stored tokens**：通过 `APITokenService` 在运行时创建，每个 token 在创建时绑定到当前 `X-Workspace-ID`：
  ...
  - Daemon credential 为 `API_TOKEN_KIND_DAEMON` + `daemon:connect` scope，只能由 `DaemonService.CreateDaemonCredential` 为已有 daemon config 签发。
  - 命中后异步更新 `last_used_at`。`RevokeAPIToken` 直接失效。
  - 用 API token 鉴权时自动覆盖请求 workspace，忽略客户端 header。
- **作用范围**：普通 API token 只能进入 HTTP/RPC API；daemon credential 只能进入 daemon gRPC，且必须匹配 daemon_id 与 workspace。
...
- `RemoteAgentService.GetRemoteAgentStatus`：A2A 拉 `/.well-known/agent.json`；DAEMON 协议在当前 workspace 查注册表，返回 ACTIVE / IDLE / UNREACHABLE + `serving_daemon_id`。
- `ChannelService.GetChannelStatus`：返回 LIVE / PAUSED / ERROR；`channel.Manager.ChannelStatus()` 当前主要看 `enabled` + manager started。
- `DaemonService.ListDaemonConfigs` / `CreateDaemonCredential`：管理 workspace daemon 资源与 worker credential。
...
- 一级页（`front/src/pages/`）：Login / Chat / Forum / Dashboard(Overview) / Agents / MCP Servers / Remote Agents / Daemons / Channels / Sessions / Cron / API Tokens / Model Providers / Notify Groups / Agent Files / Workspaces / Users / Profile / Integrations / Operations / Admin。
...
- 内置 system agent 仍为全局注册；daemon gRPC 是单端口长连接入口，但连接、registry、任务路由和配置均按 workspace 隔离。
