...
- **反向连接架构**：daemon client（`cmd/butter-daemon --url <grpc-url> --token <runtime-token>`）主动连接服务端 gRPC `DaemonConnectorService.Connect`，用 workspace-scoped runtime token 鉴权，注册 `DaemonInfo` 与支持的 `acp_runtimes`。
...
- **MongoDB**（默认数据库 `butter`，可通过 `mongo_db` 配置）：
  ...
  - `users`：dashboard 用户
  - 运行时配置：`config_agents` / `config_mcpservers` / `config_remoteagents` / `config_daemon_runtimes` / `config_channels` / `config_modelproviders` / `config_notifygroups`，`_id` 为 `"{workspace_id}:{name}"` 或 `"{workspace_id}:{id}"` 复合键
...
- **作用范围**：普通 API token 只能进入 HTTP/RPC API；daemon runtime token 只能进入 daemon gRPC，且必须匹配 daemon_runtime_id 与 workspace。
...
cmd/butter-daemon (客户端)
  └── 反连服务端 -> 本地 executor (ACP: opencode / codex; optional shell)
```
