...
>
> 保留本文作为当时的决策记录与 RPC 命名依据；不再用作工作清单。正文里出现的早期 daemon 名称（如 `DaemonConfig`、`CreateDaemonCredential`、`config_daemons`）已被当前的 `DaemonRuntime`、`CreateDaemonRuntimeToken`、`config_daemon_runtimes` 替代。新增能力请直接更新 `api.md` / `app.md` / `architecture.md`。
