...
### 2.5 与现有 SSE/流式 endpoint 的关系
- `chat_stream` / `a2a` 现在是自定义 SSE handler，**不涉及** Twirp，不在本次改造范围。
- 若以后想把 chat stream 改成 Connect server-stream，Connect 也支持，但需要先把 proto 改成 streaming RPC。本期不动。
