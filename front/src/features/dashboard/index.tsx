...
function SystemHealth({
  ...
  const problemCount = items.filter(
    (item) =>
      item?.status === 'STATUS_DEGRADED' || item?.status === 'STATUS_DOWN'
  ).length
        ...
        >
          ...
          <div className='min-w-0'>
            <p className='text-sm font-semibold'>
              {problemCount > 0
                ? `${problemCount} system issue${problemCount === 1 ? '' : 's'}`
                : 'Systems operational'}
            </p>
  ...
  return (
    <div className='flex items-center gap-3 border-b border-border px-4 py-3.5 last:border-b-0'>
      ...
      <span className='hidden items-center gap-1.5 text-xs font-medium text-running-foreground sm:flex'>
        <span className='size-1.5 animate-blink rounded-full bg-running' />
        Running
      </span>
    </div>
  )
...
export function Dashboard() {
  ...
  const failedRuns =
    (metrics?.agent_runs_failed ?? 0) + (metrics?.automation_runs_failed ?? 0)
  const totalRuns = agentRuns + automationRuns
  const successRate =
    totalRuns > 0 ? Math.round(((totalRuns - failedRuns) / totalRuns) * 100) : 100
  ...
  const infraAttention = useMemo<Attention[]>(() => {
    ...
    return components
      .filter(
        (c) =>
          c.health?.status === 'STATUS_DEGRADED' ||
          c.health?.status === 'STATUS_DOWN'
      )
      .map((c) => {
  ...
  return (
    <Page>
      ...
      />
      <PageScroll className='max-w-[1320px]'>
        {loadingOverview ? (
          ...
          <div className='space-y-5'>
            ...
            <section>
              ...
              <div className='grid grid-cols-2 overflow-hidden rounded-lg border border-transparent bg-card shadow-card [&>*:nth-child(even)]:border-l [&>*:nth-child(n+3)]:border-t sm:grid-cols-4 sm:divide-x sm:divide-border sm:[&>*:nth-child(even)]:border-l-0 sm:[&>*:nth-child(n+3)]:border-t-0'>
                <Metric
                  label='Agent runs'
                  value={agentRuns.toLocaleString()}
                  detail='Agent invocations'
                  emphasis
                />
                <Metric
                  label='Automation runs'
                  value={automationRuns.toLocaleString()}
                  detail='Scheduled executions'
                />
                <Metric
                  label='Failed runs'
                  value={failedRuns.toLocaleString()}
                  detail={failedRuns > 0 ? 'Requires review' : 'No failures'}
                />
                <Metric
                  label='Success rate'
                  value={`${successRate}%`}
                  detail={`${totalRuns.toLocaleString()} total runs`}
                />
              </div>
            </section>
              ...
              <div className='space-y-5'>
                <section>
                  ...
                  <div className='overflow-hidden rounded-lg border border-transparent bg-card shadow-card'>
                    {attentionShown.length === 0 ? (
                      <EmptyRow
                        icon={<CheckCircle2 className='text-success-foreground' />}
                      >
                ...
                <section>
                  <SectionHeader
                    ...
                    aside={
                      activeNow.length > 0 ? `${activeNow.length} running` : 'Idle'
                    }
                  />
                  <div className='overflow-hidden rounded-lg border border-transparent bg-card shadow-card'>
                    {activeNow.length === 0 ? (
                      <EmptyRow icon={<CircleDashed />}>
                        No agent runs are active
                      </EmptyRow>
                    ) : (
                      activeNow.map((event) => (
                        <ActiveRow key={event.id} event={event} />
                      ))
                    )}
                  </div>
                </section>
              ...
              <aside className='space-y-5'>
                ...
                {quickStartAgents.length > 0 && (
                  <section>
                    <SectionHeader
                      title='Start a chat'
                      description='Jump into an available agent'
                    />
