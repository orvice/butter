...
export function RemoteAgentEdit() {
  const { id } = useParams({ from: '/_authenticated/remote-agents/$id/edit' })
  ...
  function onSubmit(agent: RemoteAgent) {
    updateMutation.mutate(agent, {
      onSuccess: () => { toast.success('Remote agent updated'); navigate({ to: '/remote-agents' }) },
