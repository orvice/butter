...
function AutomationRow({
  ...
  return (
    <div
      role='button'
      tabIndex={0}
      onClick={() => navigate({ to: '/automations/$name', params: { name: job.name } })}
      onKeyDown={(e) => {
        if (e.key === 'Enter' || e.key === ' ') {
          e.preventDefault()
          navigate({ to: '/automations/$name', params: { name: job.name } })
        }
      }}
      ...
      >
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align='end' sideOffset={6}>
            <DropdownMenuItem
              onClick={() => navigate({ to: '/automations/$name/edit', params: { name: job.name } })}
            >
              <Pencil /> Edit
            </DropdownMenuItem>
            <DropdownMenuSeparator />
