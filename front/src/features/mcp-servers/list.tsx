...
export function MCPServerList() {
  ...
  const { mcp_oauth: oauthResult } = useSearch({ from: '/_authenticated/mcp-servers/' })
  ...
  useEffect(() => {
    ...
    navigate({
      to: '.',
      replace: true,
      search: (prev) => ({ ...prev, mcp_oauth: undefined, server_id: undefined }),
    })
        ...
        <div className='flex justify-end'>
          <DropdownMenu>
            ...
            <DropdownMenuContent align='end'>
              <DropdownMenuItem
                onClick={() =>
                  navigate({ to: '/mcp-servers/$id/edit', params: { id: row.id ?? '' } })
                }
              >
...
function AuthStatusCell({ server }: { server: MCPServer }) {
  ...
  if (type === 'MCP_SERVER_AUTH_TYPE_STATIC_HEADERS') {
    return (
      <Badge variant='outline' className='text-xs'>
        <KeyRound className='mr-1 h-3 w-3' /> Headers
      </Badge>
    )
  }
  if (!isOAuth) {
