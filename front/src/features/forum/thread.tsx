...
export function ForumThreadPage() {
  const { id } = useParams({ from: '/_authenticated/forum/$id' })
  ...
  async function handleAskAgent() {
    ...
    try {
      ...
      setAgentMessage('')
      toast.success(`${selectedAgentName} is processing this thread`)
    } catch (err) {
    ...
    <Page>
      ...
      <div className='border-b border-border px-4 py-4 md:px-6'>
        ...
        >
          ...
          <span className='max-w-64 truncate text-foreground'>{thread.title}</span>
        </Link>
        <div className='flex flex-col gap-3 md:flex-row md:items-center md:justify-between'>
          <div className='min-w-0'>
            ...
            <p className='mt-0.5 text-sm text-muted-foreground'>Updated {fmtDate(thread.updated_at)}</p>
          ...
          <div className='space-y-4'>
            ...
            <Card>
              ...
              <CardContent className='space-y-3'>
                ...
                <Button className='w-full' onClick={handleAskAgent} disabled={invokeAgent.isPending || isProcessing || !selectedRef}>
