...
// Hoisted so the ~25 component closures aren't reallocated on every render
// (matters during streaming, where this re-renders on each token).
const markdownComponents: Components = {
  ...
  code: ({ className: cls, children, ...props }) => {
    ...
    if (isInline) {
      return (
        <code className="bg-muted px-1.5 py-0.5 rounded font-mono text-[0.8em]" {...props}>
          {children}
        </code>
      );
    }
