...
export function WorkspaceProvider({ children }: { children: ReactNode }) {
  const isAuthenticated = useAuthStore((state) => !!state.auth.token)
  const loginWorkspaces = useAuthStore((state) => state.auth.loginWorkspaces)
  ...
  useEffect(() => {
    ...
    const persistedIsValid =
      persisted && workspaces.some((ws) => ws.id === persisted)
  ...
  const { mutateAsync: createWorkspaceAsync } = createMutation
  const createWorkspace = useCallback(
    async (input: CreateWorkspaceInput) => {
      const res = await createWorkspaceAsync(input)
      if (!res.workspace) {
