...
func NewFromProto(ctx context.Context, pb *agentsv1.Agent) (agent.Agent, error) {
	if pb == nil {
	...
	case agentsv1.AgentType_AGENT_TYPE_LLM, agentsv1.AgentType_AGENT_TYPE_UNSPECIFIED:
		return newLLMAgent(ctx, pb, subAgents)
...
func newLLMAgent(ctx context.Context, pb *agentsv1.Agent, subAgents []agent.Agent) (agent.Agent, error) {
	llmCfg := pb.GetConfig().GetLlm()
	if llmCfg == nil {
		return nil, fmt.Errorf("agent %q: LLM agent requires llm config", pb.GetName())
	}

	m, err := gemini.NewModel(ctx, llmCfg.GetModel(), &genai.ClientConfig{})
	if err != nil {
