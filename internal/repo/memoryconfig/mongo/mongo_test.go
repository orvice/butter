...
func testDB(t *testing.T) *mongo.Database {
	...
	db := client.Database(fmt.Sprintf("butter_memoryconfig_test_%d", time.Now().UnixNano()))
	t.Cleanup(func() { _ = db.Drop(context.Background()) })
	return db
}

func TestMongoRepositoryConformance(t *testing.T) {
	repotest.Run(t, func(t *testing.T) memoryconfigrepo.Repository {
		store := memoryconfigmongo.New(testDB(t))
