...
func testDB(t *testing.T) *mongo.Database {
	...
	db := client.Database(fmt.Sprintf("butter_butterbox_test_%d", time.Now().UnixNano()))
	t.Cleanup(func() { _ = db.Drop(context.Background()) })
	return db
}

func TestMongoRepositoryConformance(t *testing.T) {
	repotest.Run(t, func(t *testing.T) butterboxrepo.Repository {
		store := butterboxmongo.New(testDB(t))
