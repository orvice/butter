...
// If no override exists, it toggles relative to the channel default.
...
// IsActive checks if debug mode is active for a session.
// Per-session override takes precedence over channel config default.
func IsDebugActive(ctx context.Context, toggle *DebugToggle, channelName, sessionID string, telegramCfg *agentsv1.TelegramChannelConfig) bool {
	if toggle != nil {
