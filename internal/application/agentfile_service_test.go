...
func TestAgentFileServiceWriteEnforcesMaxFileBytes(t *testing.T) {
	...
	if _, err := svc.WriteAgentFile(ctx, connect.NewRequest(&agentsv1.WriteAgentFileRequest{
		SpaceId: space.GetId(),
		Path:    "/ok.txt",
		Content: "1234",
	})); err != nil {
	...
	_, err = svc.WriteAgentFile(ctx, connect.NewRequest(&agentsv1.WriteAgentFileRequest{
		SpaceId: space.GetId(),
		Path:    "/too-large.txt",
		Content: "12345",
	}))
