...
func TestMCPServerServiceServer_OAuthValidation(t *testing.T) {
	...
	_, err := svc.CreateMCPServer(testCtx(), connect.NewRequest(&agentsv1.CreateMCPServerRequest{McpServer: &agentsv1.MCPServer{
...
func TestMCPServerServiceServer_OAuthStatusAndDisconnect(t *testing.T) {
	...
	status, err := svc.GetMCPServerOAuthStatus(testCtx(), connect.NewRequest(&agentsv1.GetMCPServerOAuthStatusRequest{ServerId: "oauth-http"}))
	if err != nil {
		t.Fatalf("status: %v", err)
	}
	if status.Msg.GetStatus().GetState() != agentsv1.MCPOAuthConnectionState_MCPO_AUTH_CONNECTION_STATE_DISCONNECTED {
		t.Fatalf("expected disconnected, got %v", status.Msg.GetStatus().GetState())
	}
	if _, err := svc.DisconnectMCPServerOAuth(testCtx(), connect.NewRequest(&agentsv1.DisconnectMCPServerOAuthRequest{ServerId: "oauth-http"})); err != nil {
		t.Fatalf("disconnect: %v", err)
	}
	if _, err := svc.GetMCPServer(testCtx(), connect.NewRequest(&agentsv1.GetMCPServerRequest{Id: "oauth-http"})); err != nil {
...
func TestMCPServerServiceServer_ListToolsOAuthMissingConnection(t *testing.T) {
	...
	resp, err := svc.ListMCPTools(testCtx(), connect.NewRequest(&agentsv1.ListMCPToolsRequest{ServerId: "oauth-http"}))
	if err != nil {
		t.Fatalf("list tools: %v", err)
	}
	if got := resp.Msg.GetErrors()["oauth-http"]; !strings.Contains(got, "authorization") {
