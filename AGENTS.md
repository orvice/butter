...
- `internal/runtime/pibox/` — Bridge from `AGENT_TYPE_PI` leaves to a ButterBox's PiService (ADR-0011; client stubs from the `github.com/orvice/butter-box` module). Turns are asynchronous — `SubmitMessage` returns an entries cursor and `GetTurn` long-polls it, never a held connection — so a box restart mid-run yields an honest did-not-finish. One pi session per (butter session × agent), keyed in ADK session state under `pibox:<agent_id>` with the box ID and working directory it was created against; a repointed agent or a `not_found` from the box abandons and recreates, never migrates. ctx cancellation and the `max_run_seconds` deadline (default 1800, explicit 0 = unlimited) abort through one `AbortSession` path; capacity/busy map to actionable errors; user images pass through as bytes. The `ClientFactory` resolves box + decrypted token per call (no cache — a rotation takes effect on the next turn). butter never deletes data on the box.
...
- `agent.proto` — Agent tree config: `Agent`, `AgentConfig`, `LLMAgentConfig`, `MCPServer`, workflow agent configs (Loop, Sequential, Parallel), the graph Workflow Agent config (`WorkflowConfig`, `WorkflowNode`, `WorkflowEdge`, `WorkflowNodeKind`, `WorkflowRetryConfig`), and the Pi agent binding (`AGENT_TYPE_PI` + `PiAgentConfig`: butterbox_id, working_dir, provider/model/thinking_level passed through to pi, optional `max_run_seconds`). A PI agent is a leaf whose behavior surface lives on the box, so `instruction`, `global_instruction`, MCP servers/references, skills, file mounts, context guard, and remote-agent references are rejected on write; the referenced ButterBox must exist, and deleting a ButterBox is refused while PI agents (including tombstones) reference it.
